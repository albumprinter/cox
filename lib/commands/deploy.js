...
module.exports = function(args){
    ...
    var command = function(moduleName, moduleRepo, done){
        ...
        var createProjectTagAndPush = function(){

            var projectVersion = packageData[ "version" ];
            console.log("Commit changes and create and push new tag [ " + projectVersion + " ]");

            //TODO:  async... git commands!!!!

            // commit to base branch for update
            exec('git add . ');
            exec('git commit -m ' + '"Update for new tag version [ ' + projectVersion + ' ]"');
            exec('git push origin ' + baseBranch);

            // create tag
            exec('git tag ' + projectVersion);
            exec('git push origin ' + projectVersion);
        }
    };
